...
var SQL = class SQL {
	...
	get usedTables() {
		...
		this._usedTables = [];
		for (const chunk of this.queryChunks) if ((0, __entity_ts.is)(chunk, require_table.Table)) {
			const schemaName = chunk[require_table.Table.Symbol.Schema];
			this._usedTables.push(schemaName === void 0 ? chunk[require_table.Table.Symbol.Name] : schemaName + "." + chunk[require_table.Table.Symbol.Name]);
		}
	...
	buildQueryFromSourceParams(chunks, _config) {
		...
		const mappedChunks = chunks.map((chunk) => {
			...
			if (Array.isArray(chunk)) {
				const result = Array.from({ length: chunk.length * 2 + 1 });
				let writeIdx = 0;
				let readIdx = 0;
				...
				while (readIdx < chunk.length) {
					...
					result[writeIdx++] = p;
					if (readIdx < chunk.length) result[writeIdx++] = new StringChunk(", ");
				}
				result[writeIdx++] = new StringChunk(")");
