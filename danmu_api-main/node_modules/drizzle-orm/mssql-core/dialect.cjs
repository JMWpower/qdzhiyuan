	...
	*/
	...
	buildSelectionOutput(fields, { type }) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			switch (fieldType) {
				case "Column":
					chunks.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), field.isAlias ? __sql_sql_ts.sql`${__sql_sql_ts.sql.identifier((0, __alias_ts.getOriginalColumnFromAlias)(field).name)} as ${field}` : __sql_sql_ts.sql.identifier(field.name)]));
					break;
				case "SQL.Aliased":
					if (field.isSelectionField) chunks.push(__sql_sql_ts.sql.join([__sql_sql_ts.sql.raw(`${type}.`), __sql_sql_ts.sql.identifier(field.fieldAlias)]));
					else {
	...
	buildInsertQuery({ table, values, output, columnList }) {
		...
		const valuesSql = insertOrder.length === 0 ? void 0 : new __sql_sql_ts.SQL(valuesSqlList);
