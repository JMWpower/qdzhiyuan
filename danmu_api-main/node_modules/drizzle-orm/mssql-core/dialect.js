	...
	*/
	...
	buildSelectionOutput(fields, { type }) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			switch (fieldType) {
				case "Column":
					chunks.push(sql.join([sql.raw(`${type}.`), field.isAlias ? sql`${sql.identifier(getOriginalColumnFromAlias(field).name)} as ${field}` : sql.identifier(field.name)]));
					break;
				case "SQL.Aliased":
					if (field.isSelectionField) chunks.push(sql.join([sql.raw(`${type}.`), sql.identifier(field.fieldAlias)]));
					else {
	...
	buildInsertQuery({ table, values, output, columnList }) {
		...
		const valuesSql = insertOrder.length === 0 ? void 0 : new SQL(valuesSqlList);
