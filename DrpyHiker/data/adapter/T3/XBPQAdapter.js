...
function XBPQAdapter(source) {
    source.ext = runtimeConfig.getAbsolutePath(source.ext);
    this.source = source;
    this.XBPQRule = source.ext;
    if (typeof source.ext === "string") {
        //log(source.ext)
        this.XBPQRule = JSON.parse(toCorrectJSONString(fetch(source.ext)));
    }
    if (source.click) {
...
function findIndicesOfElementsWithNumbers(arr) {
    ...
    const indices = arr.map((element, index) => {
        // 检查元素是否包含数字
...
//const XBPQParse = $.require("file:///storage/emulated/0/Download/web/libs/xbpqp.js")();

Object.assign(XBPQAdapter.prototype, {
    getRuleValue(keys, def) {
        let v = "";
        for (let key of keys) {
            v = this.XBPQRule[key];
            if (v) break;
    ...
    judgeUrl(url, name, host, type) {
        ...
        if (type == "一级") {
            ...
            if (/vod\/type\/id\/\d+\.html|\/label\/(hot|down|live|web|top).html|user\/login/.test(url) || /^#$/.test(url)) {
                console.setTag("XBPQ").log("命中:", url)
                result = true;
            }
    ...
    AutoContent() {
        ...
        return {
            isEmpty: function(keys, def) {
                // 调用继承的 getRuleValue 方法
                var value = self.getRuleValue(keys, def);
                // 如果 value 是 undefined、null、空字符串或者空数组，则返回 false
                return value === undefined || value === null || value === '' || (Array.isArray(value) && value.length === 0) ? false : true;
            },
            splitStr: (html, keys, def, params) => {
                let result = null;
                ...
                for (let v of value) {
                    ...
                    if (v && XBPQParse.isParseJson(html,v)) {
            ...
            splitArray: (html, keys, def, params) => {
                params = params ? params : {};
                if (typeof def == "object") {
                    params = def;
                    def = params.def || "";
                ...
                if (list.length > 0) {
                    ...
                    results = this.loopTest(html, "数组", key);
                }
                return results;
            }
        };
        ...
        if (type == "文本") {
            ...
            if (Array.isArray(contents)) {
                for (let content of contents) {
                    ...
                    result = XBPQParse.getHasRuleSplitStr(html, content, null, strict);
                    if (key == "副标题") {
                        // log(result)
                    }
                    if (result != null) {
        ...
        if (type == "链接") {
            ...
            if (Array.isArray(contents)) {
                for (let content of contents) {
                    ...
                    if (result != null && !es.test(result)) {
                        result = collapseSpacesAndTrim(result);
                        console.setTag("XBPQ").log("test:", result)
                        if (host) {
                            if (result.startsWith("/")) {
                                result = host + result;
                            }
    ...
    init() {
        ...
        if (主分类.length == 0 && 分类数组) {
            ...
            console.setTag("XBPQ").log("获取分类中...");
            let mainurl = autoContent.isEmpty(["主页url"]) ? this.getRuleValue(["主页url"]) : this.rule.host;
            console.setTag("XBPQ").log("主页url:", mainurl);
            let html = this.req(mainurl);
        ...
        //log(ua)
        if (ua === "手机") {
            ua = "Mozilla/5.0 (Linux; Android 11; Ghxi Build/RKQ1.200826.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/76.0.3809.89 Mobile Safari/537.36";
        } else if (ua === "电脑") {
    ...
    getClassData() {
        ...
        //log(this.getRuleValue(["筛选"]))

        home.filters = this.getRuleValue(["筛选"], {});
    ...
    homeVod() {
        ...
        let list = [];
        try {
            list = JSON.parse(this.一级(url, "首页")).list;
            if (list.length > 100) {
                list = list.slice(0, 99);
            }
        } catch (e) {
            console.setTag("XBPQ").error("一级加载出错:" + e)
        }
    ...
    home() {
        return JSON.stringify(this.getClassData());
    },
    一级(url, ctype) {
        console.setTag("XBPQ").log("url:" + url)
        //let html;
        ...
        if (ctype == "首页" && !搜索url) {
            ...
            if (s) {
                if (searchmap.has(s)) {
                    console.setTag("XBPQ").log("搜索url设置:", searchmap.get(s));
        ...
        let {
            ihost,
            getUrl
        } = this.prefixORsuffix(html, "", host);

        console.setTag("XBPQ").log("ihost", host)
        let list = [];
        //log(this.getRuleValue(["数组"]))
        ...
        if (list.length && list.every(it => it.vod_id.includes("http"))) {
            let topurls = list.map(x => x.vod_id);
            //log(topurls)
            //log(topurls.length);
            //获取相同部分
            ...
            //log("相同部分")
            //log(urlpath);
            tlist = list.filter(it => it.vod_id.includes(urlpath));
            if (tlist.length > 10) {
                list = tlist
            }
        }
        ...
        if (list.length) {
            console.setTag("XBPQ").info("list:", list.slice(0, 3))
        }
    ...
    category(tid, pg, filter, extend) {
        ...
        if (url.includes("[") && (url.includes("];") || /\]$/.test(url)) || url.includes("|")) {
            url = pg == firstPage ? url.replace(/.*[\[|\|].*(http[^\]]*)\]?.*/g, "$1").replace("firstPage=", "") : url.replace(/\|\|/g, "\\|").replace(/(.*)[\[|\|].*/g, "$1");
        }
        //log("f:"+url)
        ...
        for (let [key, value] of urlMap) {
            // 在这里进行判断
            if (url.includes(key)) {
                //console.setTag("XBPQ").log(`${key}`);
                let k = key.replace(/[{}]/g, "");
                if (extend.hasOwnProperty(k) && extend[k] != "") {} else {
        ...
        //log("f:"+url)
        url = url.replace("{cateId}", extend.cateId || tid || "").replace("{class}", extend.class || "").replace("{area}", extend.area || "").replace("{year}", extend.year || "").replace("{by}", extend.by || "").replace("{lang}", extend.lang || "").replace("{catePg}", pg).replace("{letter}", extend.letter || "");
        ...
        return this.一级(url, extend.cateId || tid);
    },
    detail(vod_url) {
        //log("vod_url", vod_url)
        ...
        console.setTag("XBPQ").info('orId:' + orId);
        ...
        console.setTag("XBPQ").info("detailUrl", detailUrl)
        ...
        console.setTag("XBPQ").info("vod_name:", vod_name);
        console.setTag("XBPQ").info("vod_pic:", vod_pic);
        ...
        console.setTag("XBPQ").info("vod_url", vod_url)


        let html = this.req(vod_url);
        ...
        let 简介 = autoContent.splitStr(html, ["简介"], {
            type: "文本",
            key: "简介"
        }) || "";
        简介 = 简介.replace(/\\\\r\\\\n/g, "<br>").replace(/\\n/g, "<br>");
        ...
        let descs = {
            状态: autoContent.splitStr(html, ["影片状态"], autoRule["影片状态"], {
                key: "影片状态"
            }) || "",
            类型: autoContent.splitStr(html, ["影片类型"], autoRule["影片类型"], {
                key: "影片类型"
            }) || "",
            主演: autoContent.splitStr(html, ["主演"], autoRule["主演"][0], {
                type: "文本",
                key: "主演"
            }) || "",
            导演: autoContent.splitStr(html, ["导演"], autoRule["导演"][0], {
                type: "文本",
                key: "导演"
            }) || "",
            年代: autoContent.splitStr(html, ["影片年代"], autoRule["影片年代"], {
                key: "影片年代"
            }) || "",
            地区: autoContent.splitStr(html, ["影片地区"], autoRule["影片地区"], {
                key: "影片地区"
            }) || "",
        }
        //log(descs)
        ...
        let 多线数组 = (XBPQParse.getSplitArray(多线, this.getRuleValue(["多线数组"])) || [])
            .map(v => {
                let {
                    ihost: ithost,
                    getUrl
                } = this.prefixORsuffix(html, "多线", host);
                let url = XBPQParse.getSplitStr(v, this.getRuleValue(["多线链接"], 'href="&&"'), ithost);
                url = getUrl(url);
                return url;
            });
        if (多线数组.length) {
            console.setTag("XBPQ").info("多线数组", 多线数组)
        }
        ...
        if (多线数组.length) {
            ...
            console.setTag("XBPQ").info("多线程结束,耗时:", end_time);
        }
        //log(htmls.length)
        ...
        function getplaylist(html, playHtmlArr, i, rule) {
            ...
            let {
                ihost: phost,
                getUrl: pgetUrl
            } = rule.prefixORsuffix(html, "播放", host);
            let playlist = [];
        ...
        htmls.forEach(ithml => {
            ...
            let playHtmlArr = autoContent.splitArray(playHtml, ["播放数组"], {
                key: "播放数组"
            });

            console.setTag("XBPQ").log("playHtmlArr", playHtmlArr.length)
            //log(playHtml,playHtmlArr)
        ...
        console.setTag("XBPQ").log(lists.map(x => x.split("#").length))
        ...
        if (!from.length || !lists.length) {
            if (this.rule.类型 === "小说") {
                resitem.novelMatchUrl=vod_url;
            } else {
                console.setTag("XBPQ").info("AI匹配");
    ...
    play(flag, input) {
        ...
        if (input && jumplay) {
            let h = this.req(input, Object.assign({}, this.rule.headers, {
        ...
        console.setTag("XBPQ").info("playobj:", playobj);
    ...
    search(wd, quick, pg) {
        ...
        console.setTag("XBPQ").log(this.rule.name, "search:", surl)
        let html = this.req(surl);
        const autoContent = this.AutoContent();
        let ctype = "搜索";
        ...
        for (let item of htmlList) {
            ...
            let {
                ihost,
                getUrl
            } = this.prefixORsuffix(item, "搜索", host);
        ...
        if (list.length) {
            console.setTag("XBPQ").info("searchlist:", list.slice(0, 3))
        }
