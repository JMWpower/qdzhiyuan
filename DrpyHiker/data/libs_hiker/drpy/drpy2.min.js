//import './web/index.js'
import {cheerio, 模板} from "./drpy-core-fast.min.js";
import './fetch.js'
let vercode = typeof pdfl === "function" ? "drpy2.1" : "drpy2";
//let qjsver = `${navigator.userAgent} ${navigator.platform}`;
//const VERSION = vercode + " 3.9.53 20251026 " + qjsver;
const VERSION = vercode + " 3.9.53 20251026 ";
const UpdateInfo = [{
    date: "260115",
    title: "适配爱佬So260115版",
    version: "3.9.53 20251026",
    msg: `
增加导入sqlite.js和url.js
增加sqlite3数据库支持
支持Fs path模块
       `
},{
    ...
    version: "3.9.53 20251026",
    msg: `
更换zlib,增加支持inflate和deflate
更新gzip和ungzip
       `
},{
    date: "20251125",
    title: "适配爱佬So20251026版",
    version: "3.9.53 20251026",
    msg: `
修复ungzip
更新去广告算法1115,增加域名和路径检测
修改playParse，支持m3u8嗅探去广告
       `
},{
    date: "20251021",
    title: "适配爱佬So20251021版",
    version: "3.9.53 20251021",
    msg: `
删除多余异步
...
function uint8ArrayToBase64(uint8Array) {
    return Buffer.from(uint8Array, 'utf8').toString('base64');
}
...
function gzip(str) {
    let arr = zlib.gzip(Buffer.from(str));
    return Buffer.from(arr, 'utf8').toString('base64');
}

function ungzip(b64Data) {
    const binData = Buffer.from(b64Data, 'base64');
    const data = zlib.gunzip(binData.buffer);
...
const RSA = {
    ...
    decryptMergedData: function (privateKey, mergedData) {
        const segmentLength = (privateKey.algorithm.modulusLength + 7) >> 3;
...
function fixAdM3u8Ai(m3u8_url, headers) {
    ...
    // 辅助函数：获取URL的域名和路径
    function getUrlDomainAndPath(url, baseUrl) {
    ...
    // 辅助函数：分析URL模式（增强版）
    function analyzeUrlPatterns(urls, baseUrl) {
        ...
        // 检查是否是哈希命名
        let isHashNamed = true;
        for (let i = 0; i < Math.min(urls.length, 10); i++) {
            let url = urls[i];
            let parts = url.split('/');
            let filename = parts[parts.length - 1];
            
            if (!filename || filename.length !== 35 || !filename.endsWith('.ts') || 
                !/^[a-f0-9]+\.ts$/.test(filename)) {
                isHashNamed = false;
                break;
            }
        }
        ...
        for (let i = 0; i < urls.length; i++) {
            let url = urls[i];
            let { domain, path } = getUrlDomainAndPath(url, baseUrl);
        ...
        for (let path in pathCounts) {
            if (pathCounts[path] > maxPathCount) {
                maxPathCount = pathCounts[path];
                maxPath = path;
            }
        }
        
        let domainCount = Object.keys(domainCounts).length;
        let pathCount = Object.keys(pathCounts).length;
        
        return {
            hasClearPattern: !isHashNamed && domainCount === 1 && pathCount <= 2,
            isHashNamed: isHashNamed,
    ...
    m3u8 = m3u8.trim().split("\n").map(it => 
        it.startsWith("#") ? it : urljoin(m3u8_url, it)
    ).join("\n");
    m3u8 = m3u8.replace(/\n\n/gi, "\n");

    // 处理嵌套M3U8
    let last_url = m3u8.split("\n").slice(-1)[0];
    if (last_url.length < 5) last_url = m3u8.split("\n").slice(-2)[0];
    ...
    let patternAnalysis = analyzeUrlPatterns(tsUrls, m3u8_url);
    
    log("URL模式分析: 哈希命名=" + patternAnalysis.isHashNamed + 
        ", 域名数=" + patternAnalysis.domainCount + 
        ", 路径数=" + patternAnalysis.pathCount);
    
    let ad_urls = [];
    
    // 根据分析结果选择广告检测策略
    if (patternAnalysis.domainCount > 1 || patternAnalysis.pathCount > 1) {
        // 情况1：多域名或多路径，使用域名/路径过滤策略
        log("检测到多域名或多路径，使用域名/路径过滤策略");
        
        let loggedDomains = {};
        let loggedPaths = {};
        
        // 从后往前遍历，删除非主要域名或路径的TS片段
        for (let i = ss.length - 1; i >= 0; i--) {
            if (!ss[i].startsWith("#") && ss[i].trim().length > 0) {
                let url = ss[i];
                let { domain, path } = getUrlDomainAndPath(url, m3u8_url);
                
                let shouldDelete = false;
                
                if (patternAnalysis.domainCount > 1 && domain !== patternAnalysis.maxDomain) {
                    shouldDelete = true;
                    if (!loggedDomains[domain]) {
                        log("删除非主域名广告: " + domain);
                        loggedDomains[domain] = true;
                    }
                } else if (patternAnalysis.pathCount > 1 && path !== patternAnalysis.maxPath) {
                    shouldDelete = true;
                    if (!loggedPaths[path]) {
                        log("删除非主路径广告: " + path);
                ...
                if (shouldDelete) {
                    ad_urls.push(url);
                    if (i > 0 && ss[i-1].startsWith("#EXTINF")) {
                        ss.splice(i-1, 2);
                        i--;
                    } else {
                        ss.splice(i, 1);
        ...
        if (ad_urls.length > 0) {
            log("发现域名/路径差异型广告切片:" + ad_urls.length + "个");
        } else {
            log("未发现域名/路径差异型广告");
        }
    } else if (patternAnalysis.hasClearPattern) {
        // 情况2：非哈希命名且域名路径一致，使用URL差异检测策略
        log("使用URL差异检测策略");
        ...
        if (maxl !== lastl) {
            let reversedSs = [];
            for (let i = ss.length - 1; i >= 0; i--) {
                reversedSs.push(ss[i]);
            }
            
            for (let i = 0; i < reversedSs.length; i++) {
                let x = reversedSs[i];
                if (!x.startsWith("#")) {
        ...
        if (ad_urls.length > 0) {
            log("发现URL差异型广告切片:" + ad_urls.length + "个");
        } else {
            log("未发现URL差异型广告");
        }
    } else {
        // 情况3：哈希命名且域名路径一致，跳过URL差异检测
        log("跳过URL差异型广告检测（哈希命名且URL模式无明显差异）");
    }

    // 修复所有TS URL路径
    for (let i = 0; i < ss.length; i++) {
        if (!ss[i].startsWith("#") && ss[i].trim().length > 0) {
            ss[i] = urljoin(m3u8_url, ss[i]);
        } else if (ss[i].indexOf('URI="') !== -1 && ss[i].indexOf('URI="http') === -1) {
            ss[i] = ss[i].replace(/URI=\"([^"]+)\"/, function(match, p1) {
                return 'URI="' + urljoin(m3u8_url, p1) + '"';
            });
        }
    }
    ...
    const pattern = /(?:33333|66667)/;
    let adSegments = [];
    let currentSegment = { start: -1, count: 0, totalTime: 0, specialCount: 0, lines: [] };
    ...
    for (let i = 0; i < ss.length; i++) {
        ...
        if (line.startsWith("#EXTINF")) {
            if (currentSegment.start === -1) {
                ...
                currentSegment.lines = [];
            }
            
            let duration = parseFloat(line.slice(8));
            currentSegment.totalTime += duration;
            currentSegment.count++;
            currentSegment.lines.push(line);
            
            if (pattern.test(line)) {
                currentSegment.specialCount++;
            }
            
            if (i + 1 < ss.length && !ss[i + 1].startsWith("#")) {
                currentSegment.lines.push(ss[i + 1]);
                i++;
            }
        } 
        else if (line.startsWith("#EXT-X-DISCONTINUITY") || line.startsWith("#EXT-X-ENDLIST") || i === ss.length - 1) {
            if (currentSegment.count >= 3 && 
                currentSegment.totalTime < 30 && 
                currentSegment.specialCount >= 2) {
                
                adSegments.push({
                    start: currentSegment.start,
                    count: currentSegment.count,
                    totalTime: currentSegment.totalTime,
                    specialCount: currentSegment.specialCount,
                    lines: [...currentSegment.lines]
                });
                
                log("发现时间广告段：位置" + currentSegment.start + 
                    "，数量" + currentSegment.count + 
                    "，时长" + currentSegment.totalTime.toFixed(2) + "秒");
            }
            
            currentSegment = { start: -1, count: 0, totalTime: 0, specialCount: 0, lines: [] };
        }
    }
    
    // 新增：检查广告段是否超过阈值
    let totalAdTime = adSegments.reduce((sum, seg) => sum + seg.totalTime, 0);
    
    // 如果广告段超过12处且总时长超过4.5分钟，则跳过删除
    if (adSegments.length > 12 && totalAdTime > 270) {
        log(`时间广告段数量${adSegments.length}处，总时长${totalAdTime}秒，超过阈值，视同无时间广告，跳过删除`);
    } else if (adSegments.length > 0) {
        // 从后向前删除广告段（原逻辑）
        adSegments.sort((a, b) => b.start - a.start);
        
        let totalDeleted = 0;
        for (let i = 0; i < adSegments.length; i++) {
            let segment = adSegments[i];
            const deleteCount = segment.lines.length;
            
            if (segment.start + deleteCount <= ss.length) {
                log("删除时间广告段：位置" + segment.start + "，数量" + segment.count);
                ss.splice(segment.start, deleteCount);
                totalDeleted += segment.count;
            }
        }
        
        log("共处理 " + adSegments.length + " 处时间特征型广告，删除 " + totalDeleted + " 个片段");
    } else {
        log("未发现时间特征型广告");
    }
...
function playParse(playObj) {
    ...
    if (!rule.play_parse || !rule.lazy) {
        ...
        try {
            ...
            // encodeURIComponent二次编码处理Base64编码前的特殊字符
            // lazy_code = lazy_code + ';if (rule.skipproxyad!==true && rule.proxy_rule.indexOf(\'fixAdM3u8Ai(\')>0){let getProxyUrlStr=getProxyUrl();if(typeof input===\'string\'){if (input.indexOf(getProxyUrlStr)<0&&(/\\.m3u8$/.test(input)||input.indexOf(\'m3u8?\')>0)){let safeBase64=encodeURIComponent(base64Encode(input));input = {jx: 0,url: getProxyUrlStr+\'&url=\'+safeBase64,parse: 0,header: rule.headers};}}else{let inputUrlStr=input.url;let inputHeader=(typeof input.header===\'undefined\') ? rule.headers:input.header;if (inputUrlStr.indexOf(getProxyUrlStr)<0&&(/\\.m3u8$/.test(inputUrlStr)||inputUrlStr.indexOf(\'m3u8?\')>0)){let safeBase64=encodeURIComponent(base64Encode(inputUrlStr));input = {jx: 0,url: getProxyUrlStr+\'&url=\'+safeBase64,parse: 0,header: inputHeader};}}};';
            // 修改为必须以.m3u8结尾
            lazy_code = lazy_code + ';if (rule.skipproxyad!==true && rule.proxy_rule.indexOf(\'fixAdM3u8Ai(\')>0){let getProxyUrlStr=getProxyUrl();if(typeof input===\'string\'){if (input.indexOf(getProxyUrlStr)<0&&(/\\.m3u8$/.test(input))){let safeBase64=encodeURIComponent(base64Encode(input));input = {jx: 0,url: getProxyUrlStr+\'&url=\'+safeBase64,parse: 0,header: rule.headers};}}else{let inputUrlStr=input.url;let inputHeader=(typeof input.header===\'undefined\') ? rule.headers:input.header;if (inputUrlStr.indexOf(getProxyUrlStr)<0&&(/\\.m3u8$/.test(inputUrlStr))){let safeBase64=encodeURIComponent(base64Encode(inputUrlStr));input = {jx: 0,url: getProxyUrlStr+\'&url=\'+safeBase64,parse: 0,header: inputHeader};}}};';
...
/*
    // 新增：无论是否存在lazy或proxy_rule，只要最终返回的播放地址以.m3u8结尾，就执行指定的代理逻辑
    if (lazy_play.url && /\.m3u8$/.test(lazy_play.url)) {
        try {
            // 检查是否已经是代理地址，避免重复代理
            if (lazy_play.url.indexOf(getProxyUrl()) === -1) {
                lazy_play = {
                    parse: 0,
                    url: getProxyUrl() + '&url=' + encodeURIComponent(base64Encode(lazy_play.url)),
                    jx: 0,
                    flag: lazy_play.flag, // 保留原有flag
                    header: lazy_play.header // 保留原有header
                };
                print("强制代理M3U8地址: " + lazy_play.url);
            }
        } catch (e) {
            print(`强制代理M3U8错误: ${e.message}`);
        }
    }
*/
...
function getOriginalJs(js_code) {
    ...
    function aes_decrypt(data) {
        ...
        const key = crypto.subtle.importKey("raw", keyArray, {
            name: "AES-CBC"
        }, false, ["decrypt"]);
        try {
