...
if (ConfigManager.getGlobal("firstopen") == MY_RULE.version) {
    $.require("home");
} else if (MY_PAGE === 1) {
    let d = []
    let time = ConfigManager.getGlobal("firstopen") ? 6 : 20;
    let id = Date.now();
    let content = `
    ...
    d.push({
        ...
        title: `当前版本：${MY_RULE.version} ${ConfigManager.getGlobal("firstopen") && ConfigManager.getGlobal("firstopen") < MY_RULE.version ? "🆕" : ""}`,
        col_type: "text_1",
    ...
    updateItem(id + "timer", {
        ...
        url: $("#noLoading#").lazyRule((v) => {
            const ConfigManager = $.require('./subpage/ConfigManager.js');
            ConfigManager.setGlobal("firstopen", String(v));
